...
aU=dfNew[dfNew["stimulus"].str.contains("HHHKHHH|CCCSCCC", na=False)].index.to_numpy() 
aD=dfNew[dfNew["stimulus"].str.contains("CCCHCCC|HHHSHHH", na=False)].index.to_numpy() 

aRC=dfNew[dfNew["stimulus"].str.contains("CCCSCCC", na=False)].index.to_numpy() 
aLC=dfNew[dfNew["stimulus"].str.contains("HHHKHHH", na=False)].index.to_numpy() 
aRI=dfNew[dfNew["stimulus"].str.contains("HHHSHHH", na=False)].index.to_numpy() 
aLI=dfNew[dfNew["stimulus"].str.contains("CCCHCCC", na=False)].index.to_numpy() 

dfRC=dfNew["stimulus"].str.contains("CCCSCCC")
dfLC=dfNew["stimulus"].str.contains("HHHKHHH")
dfRI=dfNew["stimulus"].str.contains("HHHSHHH")
dfLI=dfNew["stimulus"].str.contains("CCCHCCC")
...
outString='flankerLetters_OxySoft_NIRS_Timestamp_'+str(outNums)+'_UTC.csv'
df.to_csv(outString, sep=',', encoding='utf-8', header='true')
print('Flanker (Letters) Synched')
